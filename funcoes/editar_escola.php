...
if(isset($_POST['editar'])){
	...
	if($dimensoes[0] > $lagura || $dimensoes[1] > $altura || $imagem["size"] > $tamanho){
		...
		$sql = "UPDATE escola set nome = '{$nome}', inep = '{$inep}', login = '{$login}', senha = '{$senha}' where escola_id = '{$escola_id}'";
	...
	if(count($error) == 0){
		...
		$sql = "UPDATE escola set nome = '{$nome}', inep = '{$inep}', login = '{$login}', senha = '{$senha}', imagem = '{$nome_imagem}' where escola_id = '{$escola_id}'";
	if(mysqli_query($conexao, $sql)){
